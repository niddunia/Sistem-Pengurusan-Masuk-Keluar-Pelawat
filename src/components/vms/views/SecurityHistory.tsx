...
export function SecurityHistory() {
  ...
  // Date range filter (client-side, since list endpoint returns 200 most recent)
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    a.download = `vms-history-${format(new Date(), "yyyy-MM-dd")}.csv`;
    a.click();
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-white/70 mt-1">
            {language === "bm"
              ? "Rekod semua lawatan — tapis & cari untuk maklumat terperinci"
              : "All visit records — filter & search for details"}
          </p>
        </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-white/10 hover:bg-transparent">
                  ...
                  <TableHead className="text-white/70 font-semibold">{t("hostStaff", language)}</TableHead>
                ...
                {paginated.map((v) => (
                  ...
                  >
                    ...
                    <TableCell className="text-white/80">
                      {v.hostStaff?.fullName || "-"}
                      {v.hostStaff?.department && (
                        <div className="text-xs text-white/50">{v.hostStaff.department.name}</div>
                      )}
                    </TableCell>
                    <TableCell className="text-xs text-white/90">
